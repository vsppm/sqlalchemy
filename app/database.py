...
class DataTable(object):
    ...
    def __init__(
        ...
        for f in self.filterable:
            self.selected_filter = request.values.get(f.key, None)
            self.filter(f.key, self.selected_filter)
        self.search(self.selected_query)
        self.sort(self.selected_sort, self.selected_order)
        self.count = len(self.query.all())
        if self.selected_paginate:
    ...
    def get_selected(self, request):
        ...
        self.selected_paginate = request.values.get(
    ...
    def filter(self, field, value):
        ...
        if field and value:
            ff = getattr(self.model, field)
            self.query = self.query.filter(ff == value)
    ...
    def total(self):
        """
        :return: this total number after sort filter
        """
        return self.count
